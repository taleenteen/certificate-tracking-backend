...
case "${1:-up}" in
  ...
  verify)
    ...
    echo
    echo "Backend health:"
    curl -fsS "http://localhost:${PRODUCTION_BACKEND_PORT:-3001}/docs-json" >/dev/null
    echo "  OK http://localhost:${PRODUCTION_BACKEND_PORT:-3001}/docs-json"
    echo
    echo "Frontend health:"
    curl -fsS "http://localhost:${PRODUCTION_FRONTEND_PORT:-3003}" >/dev/null
    echo "  OK http://localhost:${PRODUCTION_FRONTEND_PORT:-3003}"
    echo
    echo "Seed counts:"
